...
def detect_csv_delimiter(file_path, sample_size=2048):
    """Detect CSV delimiter by analyzing file sample"""
    try:
        ...
        try:
            ...
            delimiters = [',', ';', '\t', '|']
            counts = [sample.count(d) for d in delimiters]
            max_idx = counts.index(max(counts))
            return delimiters[max_idx] if max(counts) > 0 else ','
    except Exception as e:
        logger.error(f"Error detecting delimiter: {e}")
        return ','  # Default fallback

def safe_read_csv(file_path, **kwargs):
    """Safely read CSV with automatic delimiter detection"""
    if 'sep' not in kwargs and 'delimiter' not in kwargs:
        delimiter = detect_csv_delimiter(file_path)
        kwargs['sep'] = delimiter
        logger.info(f"Auto-detected delimiter '{delimiter}' for {os.path.basename(file_path)}")
...
def preview_file(filename):
    ...
    try:
        ...
        # FIXED: Use safe_read_csv with auto-detection
        df = safe_read_csv(file_path, nrows=100)
        
        # Get row count using same delimiter
        delimiter = detect_csv_delimiter(file_path)
        row_count = len(safe_read_csv(file_path, usecols=[0]))  # More efficient
        ...
        return render_template('data/preview.html',
                             ...
                             delimiter=repr(delimiter))  # Show detected delimiter
...
def preprocess_data():
    ...
    if request.method == 'POST':
        try:
            ...
            # ADDED: Detect delimiter before processing
            delimiter = detect_csv_delimiter(input_file)
            logger.info(f"Using delimiter '{delimiter}' for preprocessing {filename}")

            # Create preprocessor
            preprocessor = ForexDataPreprocessor(os.path.dirname(input_file))

            # MODIFIED: Pass delimiter to load_data
            preprocessor.load_data(filename, delimiter=delimiter)
